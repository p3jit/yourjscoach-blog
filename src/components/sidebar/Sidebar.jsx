...
const Sidebar = ({ isOpen, onClose }) => {
    ...
    <div className="border-b border-zinc-700/50">
      <button
        className={`flex justify-between items-center w-full py-4 px-5 text-left transition-colors ${
          expandedSections[section] ? "bg-zinc-700/30 text-white" : "text-zinc-300 hover:bg-zinc-700/20"
        }`}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <span className="bg-zinc-700 text-zinc-300 text-xs px-2 py-0.5 rounded-full">{items.length}</span>
        </div>
          ...
          {items.length > 0 ? (
            <ul className="max-h-72 overflow-y-auto py-2 scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-zinc-800 my-2 mr-2">
              {items.map((item) => {
                ...
                const isCurrentProblem = currentProblem && currentProblem.documentId === item.documentId;
                const isSolved = solvedProblems.includes(item.documentId);

                return (
                  <li
                    onClick={() => navigate(`/practice/${item.documentId || item.displayId}`)}
                    key={item.id}
                    className={`px-5 py-2.5 hover:bg-zinc-700/30 border-l-2 transition-all cursor-pointer ${
                      isCurrentProblem ? "border-zinc-400 bg-zinc-700/50" : "border-transparent hover:border-zinc-500"
                    }`}
                  >
  ...
  return (
    <div className={`flex w-full absolute z-50 h-full ${!isOpen ? "pointer-events-none" : ""}`}>
      ...
      <div
        className={`bg-zinc-800 h-full min-w-[500px] w-4/12 flex flex-col overflow-hidden shadow-xl 
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          <AccordionItem title="DSA Problems" items={dsaProblems} section="dsa" icon="◆" />
          <AccordionItem title="JavaScript Problems" items={jsProblems} section="js" icon="◆" />
          <AccordionItem title="System Design Problems" items={systemDesignProblems} section="systemDesign" icon="◆" />
        </div>
